...
框架在非调试模式下的通用错误文案支持多语言。`config/app.php` 中的 `error_message` 配置值会被当作翻译键使用：若存在对应翻译则返回翻译值，否则原样返回配置值。

例如将配置设为自定义键 `errors.server_error`：
...
若不配置 i18n 或未定义该键，则直接使用 `error_message` 的配置值作为文案。
